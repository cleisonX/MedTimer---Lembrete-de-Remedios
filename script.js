...
function exibirAcoesAdicionais() {
    ...
    acoesDiv.innerHTML = `
        ...
        <div style="display: flex; flex-wrap: wrap; gap: 10px; margin-top: 12px;">
            <button id="whatsappBtn" class="btn-whats" style="flex: 1;">Enviar no WhatsApp 📱</button>
            <button id="exportarICSBtn" style="background:#3182ce; color:white; font-weight:bold; border:none; padding:14px 20px; border-radius:40px; cursor:pointer; flex: 1;">Baixar ICS 📅</button>
        </div>
        <div style="display: flex; flex-wrap: wrap; gap: 10px; margin-top: 12px;">
            <button id="pushBtn" class="btn-push" style="flex: 1;">Ativar Notificações 🔔</button>
            <button id="copiarListaBtn" style="background:#4a5568; color:white; font-weight:bold; border:none; padding:14px 20px; border-radius:40px; cursor:pointer; flex: 1;">Copiar texto 📋</button>
        </div>
    `;
    ...
    document.getElementById('pushBtn')?.addEventListener('click', () => agendarNotificacoes());
}
...
function agendarNotificacoes() {
    if (!ultimosEventos || ultimosEventos.length === 0) {
        mostrarToast("⚠️ Calcule os horários primeiro");
        return;
    }
    ...
    if (Notification.permission === "denied") {
        mostrarToast("❌ Você bloqueou as notificações. Permita nas configurações do navegador.");
        return;
    }
    ...
    if (Notification.permission !== "granted") {
        Notification.requestPermission().then(permission => {
            if (permission === "granted") {
                ...
                mostrarToast("❌ Permissão negada. Não será possível enviar lembretes.");
            }
        });
    } else {
        agendarTodasNotificacoes();
...
function agendarTodasNotificacoes() {
    ...
    if (window.notificacoesAgendadas) {
        window.notificacoesAgendadas.forEach(timeout => clearTimeout(timeout));
    }
    window.notificacoesAgendadas = [];

    ultimosEventos.forEach(evento => {
        const tempoEvento = evento.inicio.getTime();
        if (tempoEvento > agora.getTime()) {
            const delay = tempoEvento - agora.getTime();
            ...
            window.notificacoesAgendadas.push(timeoutId);
            agendadas++;
        }
    });

    mostrarToast(`🔔 ${agendadas} notificações agendadas para as próximas doses!`);
}
